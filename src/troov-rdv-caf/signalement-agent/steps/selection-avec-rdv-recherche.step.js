...
Given("La page \"Enregistrer l’usager\" avec rendez-vous est ouverte", async function() {
...
Then("Passe à l'etape suivant: \"L’usager a été ajouté dans la file d’attente\" s'affiche sur la page confirmation avec RDV", async function() {
...
//        "L’usager a été ajouté dans la file d’attente",
//        "Vous êtes en dehors du délai prévu pour vous présenter à votre rendez-vous. Pour rappel votre RDV est prévu à",
//    ];
//
//    const pageText = await this.backofficePage.textContent('.p-message-info');
...
Then("Message d'erreur 'Aucun rendez-vous' s'affiche", async function() {
    const heading = await this.backofficePage.getByText("Aucun rendez-vous");
    await expect(heading).toBeVisible();
});
