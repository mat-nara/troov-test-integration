...
Given("Il modifie la date et l'heure du rendez-vous", async function() {
    ...
    console.log('currentHeureRdv', currentHeureRdv);
...
//      await selectorHeureDuRDV.click();
//      // Naviguer avec les touches fléchées pour sélectionner l'option
//      for (let i = 0; i <= randomIndex; i++) {
//          await this.backofficePage.keyboard.press('ArrowDown');
//          await this.backofficePage.waitForTimeout(200);
//          console.log(await selectorHeureDuRDV.locator('option').nth(i).getAttribute('value'))
//      }
// 
//      // --- Appuyer sur Entrée pour sélectionner
//      await this.backofficePage.keyboard.press('Enter');
//      await this.backofficePage.waitForTimeout(2000);
        ...
        console.log('secondOptionValue', secondOptionValue);
...
When("Il clique sur le bouton \"Modifier\" de la fiche de rendez-vous.", async function() {
    // const modifierButton = this.backofficePage.locator('button:has-text("Modifier")');
    const modifierButton = this.backofficePage.locator('button').filter({ hasText: "Modifier" }).first();
    await modifierButton.waitFor(async (btn) => !(await btn.getAttribute('disabled')), { timeout: 5000 });
    await modifierButton.click();
});
...
Then("L'historique affiche les informations correspondant à la prise et à la modification du rendez-vous", async function() {
    ...
    // const regex = /Pris le (\d{2}\/\d{2}\/\d{4}) à (\d{2}:\d{2}) par ([\w\s]+?)(?: \(([^)]+)\))? - Date de réservation : le (\d{2}\/\d{2}\/\d{4}) de (\d{2}:\d{2}) à (\d{2}:\d{2}) -\s*(.*?)?\s*- (\d+) minutes - Mode : (.+)/;
    const regex =    /Pris le (\d{2}\/\d{2}\/\d{4}) à (\d{2}:\d{2}) par ([\w\s]+?) - Date de réservation : le (\d{2}\/\d{2}\/\d{4}) de (\d{2}:\d{2}) à (\d{2}:\d{2}) -\s*(.*?)\s*-\s*(\d+) minutes - Mode : (.+)/;
    // const regex =    /Pris le (\d{2}\/\d{2}\/\d{4}) à (\d{2}:\d{2}) par ([\w\s]+?) - Date de réservation : le (\d{2}\/\d{2}\/\d{4}) de (\d{2}:\d{2}) à (\d{2}:\d{2}) -\s*(?:- )?\s*(.*?)\s*-\s*(\d+) minutes - Mode : (.+)/;
    // const regex = /Pris le (\d{2}\/\d{2}\/\d{4}) à (\d{2}:\d{2}) par ([\w\s]+?) - Date de réservation : le (\d{2}\/\d{2}\/\d{4}) de (\d{2}:\d{2}) à (\d{2}:\d{2}) -\s*-*\s*(.*?)\s*-\s*(\d+) minutes - Mode : (.+)/;
    ...
    if (match) {
        ...
        const appointmentDate = match[1];
        const appointmentTime = match[2];
        const userName = match[3];
        const reservationDate = match[4];
        const reservationStartTime = match[5];
        const reservationEndTime = match[6];
        const waitingReason = match[7];   // "" ou texte
        const duration = match[8];
        const mode = match[9];
...
Then("Le rendez-vous apparaît dans la pochette de déplacement sous le bouton \"Déplacer des RDVs\"", async function() {
    ...
    var fullname = this.name.toUpperCase() + ' ' + this.firstname
    console.log("fullname: ", fullname)
    //expect(await this.backofficePage.locator('.move-rdv .container strong').filter({ hasText: fullname })).toBeVisible();
