Feature: Gestion de l'annonce (declarer comme retrouve ou supprimer)
    ...
    Scenario: Confirmation de l'objet comme retrouve
        Given La pop-up "Confirmation objet retrouve" est affichee
        When L'utilisateur clique sur le bouton "Objet rendu"
        Then Le statut de l'annonce passe a "Trouve"
            * La pop-up se ferme

    Scenario: Annulation de la declaration "objet retrouve"
        Given La pop-up "Confirmation objet retrouve" est affichee
    ...
    Scenario: Suppression d'une annonce a tout moment
        # Non confirme visuellement sur les captures fournies - a valider avec une capture du bouton/de l'action reelle
        Given L'utilisateur est connecte a son espace usager
