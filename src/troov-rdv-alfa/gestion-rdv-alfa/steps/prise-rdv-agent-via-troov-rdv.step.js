...
const LoginPage = require('../../signalement-usager/pages/LoginPage.js');
const { generateRandomNIR, generateRandomPhone } = require('../../signalement-usager/utils/helper.js');
...
async function connexion(world) {
...
Given("Un utilisateur a été créé lors d'une prise de rendez-vous précédente", async function() {
    ...
    // Connexion au backoffice Troov RDV
    
    await this.tempPage.goto(config.troovPublicUserBackofficeURL);
    await this.tempPage.waitForTimeout(2000);

    await this.tempPage.locator('#email').fill(config.troovPublicUserBackofficeUsername);
    await this.tempPage.locator('#password').fill(config.troovPublicUserBackofficePassword);
    await this.tempPage.locator('button[type="submit"]').first().click();
    await this.tempPage.waitForTimeout(2000);
    if (await this.tempPage.locator('#password').isVisible()) {
        await this.tempPage.locator('#email').fill(config.troovPublicUserBackofficeUsername);
        await this.tempPage.locator('#password').fill(config.troovPublicUserBackofficePassword);
        await this.tempPage.locator('button[type="submit"]').first().click();
    }
...
Then("Les notes internes sont bien enregistrées dans la fiche du rendez-vous", async function() {
    ...
    //await this.backofficePage.waitForFunction(() => {
    //    const select = document.querySelector('[data-testid="dashboard"]');
    //    return select.value !== '';
    //});

    await this.backofficePage.waitForTimeout(2000);
    //await expect(this.backofficePage.getByPlaceholder('Écrire un commentaire')).not.toHaveValue('');
    //await expect(this.backofficePage.getByPlaceholder('Écrire un commentaire')).toHaveValue(v => v && v.trim().length > 0, {timeout: 15000});
    //await expect(this.backofficePage.getByPlaceholder('Écrire un commentaire')).toHaveText(/.+/, { timeout: 15000 });
    //const textarea = await this.backofficePage.getByPlaceholder('Écrire un commentaire').elementHandle();
    //await this.backofficePage.waitForFunction(textarea.value !== '');

    //const notes = await this.backofficePage.getByPlaceholder('Écrire un commentaire').inputValue();
   // console.log('notes: ', notes)
    // expect(notes).toBe(this.note);
    await expect(this.backofficePage.getByPlaceholder('Écrire un commentaire')).toHaveValue(this.note);
